...
async function main(): Promise<void> {
  ...
  console.log("UK autonomous outreach — OSM/DDG discovery, email scrape, contact forms\n");

  await runMigrations();
  const jobId = await createJob("uk_autopilot", { limit: limit ?? null });
  await updateJob(jobId, { status: "running", progress: "Autopilot running…" });

  let summary: Awaited<ReturnType<typeof runUkAutonomousOutreachBatch>> | null = null;
  try {
    summary = await runUkAutonomousOutreachBatch({ limit });
